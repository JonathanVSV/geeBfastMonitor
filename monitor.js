...
/////////////////////////////////////////////////////
var bfastMonitor = function bfastMonitor(roi,historyStart,historyEnd,
                                            monitoringStart,monitoringEnd,h,period,
                                            alpha,magnitudeThreshold,harmonics){
...
var addNDVI = function(image) {
  return image
    .addBands(image.normalizedDifference(['SR_B4', 'SR_B5'])
    .rename('NDMI'))
    .float();
};
...
var addNDVI8 = function(image) {
  return image
    .addBands(image.normalizedDifference(['SR_B5', 'SR_B6'])
    .rename('NDMI'))
    .float();
};
...
var landsat5Collection1 = landsat5Collection
  ...
  //.map(cloudMaskL457)
     .map(function(image) {
      ...
      // Aquí un cambio  
      var maskIm = image.select('QA_PIXEL');
  
      maskIm = maskIm.bitwiseAnd(dilatCloud).eq(0)
                 .and(maskIm.bitwiseAnd(cloud).eq(0))
                 .and(maskIm.bitwiseAnd(shadow).eq(0));
                                          
       return image.mask(maskIm)                             //.clip(PolygonBuffer)
                        }
      )
   .map(addNDVI);
...
var landsat8Collection1 = landsat8Collection
  ...
  //.map(maskL8sr)
     .map(function(image) {
      ...
      maskIm = maskIm.bitwiseAnd(dilatCloud).eq(0)
                 .and(maskIm.bitwiseAnd(cirrus).eq(0))
                 .and(maskIm.bitwiseAnd(cloud).eq(0))
                 .and(maskIm.bitwiseAnd(shadow).eq(0));
...
var landsat9Collection1 = landsat9Collection
  .filterBounds(roi)
  .filterDate(historyStart, monitoringEnd)
  //.map(maskL8sr)
     .map(function(image) {
...
.select('SR_B1', 'SR_B2','SR_B3', 'SR_B4','SR_B5', 'SR_B7','NDMI')
.map(function(image){
  return image.rename(['R', 'G','B', 'NIR','SWIR1', 'SWIR2','NDMI']);
});
...
.select('SR_B2', 'SR_B3','SR_B4', 'SR_B5','SR_B6', 'SR_B7','NDMI')
.map(function(image){
  return image.rename(['R', 'G','B', 'NIR','SWIR1', 'SWIR2','NDMI']);
});

var landsat9Collection01 = landsat9Collection1
.select('SR_B2', 'SR_B3','SR_B4', 'SR_B5','SR_B6', 'SR_B7','NDMI')
.map(function(image){
  return image.rename(['R', 'G','B', 'NIR','SWIR1', 'SWIR2','NDMI']);
});
...
var collection_merge2 = collection_merge.merge(landsat8Collection01).merge(landsat9Collection01);
...
var residuals = fittedHarmonic.map(computResiduals)
                              ...
                              //.aside(Map.addLayer, {bands: ['cos_1', 'sin_1', 'cos_2']}, 'residuals');
...
function makeDate(image) {
  ...
  var mask = image.select(dependent)
                  .unmask(-9999)
                  .neq(-9999);
  return md.updateMask(mask);
}
...
criticalBorder1 = criticalBorder.arraySlice(0,0,1).toArray();
...
var Cnk1 = cox.updateMask(cox.arrayGet([0,0]).neq(0)).toArray();
var timeCnk1 = Timecox.updateMask(Timecox.arrayGet([0,0]).neq(0)).toArray();
var firtsChnk = firsttChab.updateMask(firsttChab.arrayGet([0,0]).neq(0)).toArray();
...
var b1_median_mon = MonitorLandsat.select('B')
    .median();
var b1_median_his = histCollection.select('B')
    .median();
    
var b2_median_mon = MonitorLandsat.select('G')
    .median();
var b2_median_his = histCollection.select('G')
    .median();   
    
var b3_median_mon = MonitorLandsat.select('R')
    .median();
var b3_median_his = histCollection.select('R')
    .median(); 
    
var b4_median_mon = MonitorLandsat.select('NIR')
    .median();
var b4_median_his = histCollection.select('NIR')
    .median(); 

var b5_median_mon = MonitorLandsat.select('SWIR1')
    .median();
var b5_median_his = histCollection.select('SWIR1')
    .median(); 
    
var b7_median_mon = MonitorLandsat.select('SWIR2')
    .median();
var b7_median_his = histCollection.select('SWIR2')
    .median();  
...
var bfastMonitorCustom = function bfastMonitorCustom(dict){

var roi = dict.roi,
    collection = dict.collection,
    dependBand = dict.dependBand,
    auxbands = dict.auxbands,
    historyStart = dict.historyStart,
    historyEnd = dict.historyEnd,
    monitoringStart = dict.monitoringStart,
    monitoringEnd = dict.monitoringEnd,
    h = dict.h,
    period = dict.period,
    alpha = dict.alpha,
    magnitudeThreshold = dict.magnitudeThreshold,
...
harmonics = harmonics||1;
// The dependent variable we are modeling.
var dependent = dependBand||'NDVI';
var auxBands = auxbands||['B', 'G', 'R', 'NIR', 'SWIR1', 'SWIR2'];
// image Collection
var collection_merge3 = collection;
...
// Compute temporal metrics for prediction
var bandsRatioMedian = ee.List(auxBands).add(dependent).map(function(band){
  return histCollection.select(ee.String(band)).median()
                       .divide(MonitorLandsat.select(ee.String(band)).median())
                       .rename(ee.String(band));
});

bandsRatioMedian = ee.ImageCollection.fromImages(bandsRatioMedian)
    .toBands();
...
var band_metrics = bandsRatioMedian
.addBands(ndmires_ratio_median)
.addBands(firstChaDif.toFloat())
.addBands(Cnk.toFloat());
...
exports.bfastMonitorCustom = bfastMonitorCustom;
